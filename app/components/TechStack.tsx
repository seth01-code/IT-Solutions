...
export default function TechStack() {
  ...
  return (
    <section
      id="stack"
      className="py-[120px] bg-[var(--bg2)] text-[var(--text)] transition-colors duration-300"
    >
      <div className="max-w-[1200px] mx-auto px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {techCategories.map((cat) => {
            ...
            return (
              <div
                key={cat.label}
                className="fade-up bg-[var(--card)] border border-[var(--border)] rounded-2xl p-7 transition-colors"
              >
                <div className="flex items-center gap-2 mb-4">
                  <Icon className="text-[var(--purple-light)] text-lg" />
                ...
                <div className="flex flex-wrap gap-2">
                  {cat.tags.map((tag) => (
                    <span
                      key={tag}
                      className={`px-3.5 py-1.5 rounded-lg text-[13px] font-medium ${cat.tagClass}`}
                    >
                      {tag}
                    </span>
                  ))}
        ...
        {/* MARQUEE */}
        <div className="overflow-hidden my-14 py-6">
            ...
            {[...marqueeItems, ...marqueeItems].map((item, i) => {
              ...
              return (
                <div key={i} className="flex items-center gap-3 flex-shrink-0">
                  <div className="w-10 h-10 rounded-xl bg-[var(--card)] border border-[var(--border)] flex items-center justify-center text-[var(--purple-light)]">
                    <Icon />
                  </div>

                  <span className="text-[var(--text)]/80 text-sm font-medium">
                    {item.label}
                  </span>
                </div>
              );
